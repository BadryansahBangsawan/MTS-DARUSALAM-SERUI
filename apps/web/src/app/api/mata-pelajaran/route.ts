...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await db
      .select()
      .from(mataPelajaran)
      .where(or(like(mataPelajaran.name, `%${search}%`), like(mataPelajaran.description, `%${search}%`)))
...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.insert(mataPelajaran).values(body);
    const created = await db.select().from(mataPelajaran).orderBy(mataPelajaran.createdAt).limit(1);
